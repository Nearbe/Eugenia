...
from loaders import load_mnist_data, load_png_image, load_cmyk_image, load_fashion_data
...
def _render_single_visualization(module_name: str, viz_directory: str, data: VisualizationData,
    ...
    # Добавление основной директории исходного кода и папки утилит в пути поиска Python.
    # Это необходимо для того, чтобы подгружаемые модули визуализации могли
    # находить вспомогательные модули (params, models) и утилиты через обычный импорт.
    src_dir = Path(viz_directory).parent
...
def run_all_visualizations(source_name: Optional[str] = None,
    ...
    if output_directory is None:
        output_directory = os.environ.get("VIZ_OUTPUT_DIR", get_script_directory())
