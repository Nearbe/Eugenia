...
from nucleus_duality import UnifiedSystem, DualState
from nucleus_graphics import GeometricEngine, GeometricProfile, RenderParams
from nucleus_model_patterns import ModelLoader, PatternExtractor, ModelProfile
...
class NucleusUnified:
    ...
    def load_model(self, model_path: str) -> bool:
        ...
        print(f"\n[Nucleus] Загрузка модели: {model_path}")
...
def demo_unified():
    ...
    print("=" * 60)
    print("NUCLEUS UNIFIED SYSTEM")
    print("=" * 60)

    # Создаём систему
    nucleus = NucleusUnified(k=16)

    # Тест генерации (без модели)
    print("\n1. Генерация (без модели):")
    img = nucleus.generate(
    ...
    # Регистрируем
    nucleus.model_profiles["test"] = profile
    nucleus.graphics.register_profile("test", nucleus._profile_to_geo(profile))

    # Фидбек
    print("\n3. Runtime learning:")
    for i in range(5):
        nucleus.generate("запрос", mode=TaskMode.INFERENCE)
        nucleus.learn_from_feedback(success=True, boost=0.05)

    state = nucleus.get_state()
