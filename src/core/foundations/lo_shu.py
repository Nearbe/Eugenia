...
class Direction(StrEnum):
    """Fixed compass directions of the Lo Shu grid."""
...
class LoShuPosition:
    ...
    @property
    def binary_history(self) -> tuple[int, int, int, int]:
...
class LoShuAddress:
    ...
    def history(self) -> tuple[int, ...]:
        ...
        return self.position.binary_history + digit_position(operation_digit(self.operation)).binary_history
...
POSITIONS: Final[tuple[LoShuPosition, ...]] = (
    ...
    LoShuPosition(Direction.NORTH, 9, 0, 1),
    LoShuPosition(Direction.NORTH_EAST, 2, 0, 2),
    LoShuPosition(Direction.WEST, 3, 1, 0),
    LoShuPosition(Direction.CENTER, 5, 1, 1),
...
OPERATION_DIGITS: Final[dict[LoShuOperation, int]] = {
    ...
    LoShuOperation.BRANCH: 9,
    LoShuOperation.COMPRESS: 1,
    LoShuOperation.LOG_DEPTH: 3,
...
def rows() -> tuple[tuple[int, int, int], ...]:
    return GRID


def columns() -> tuple[tuple[int, int, int], ...]:
    return tuple(tuple(row[column] for row in GRID) for column in range(LO_SHU_SIZE))


def diagonals() -> tuple[tuple[int, int, int], tuple[int, int, int]]:
    return (
...
def equations() -> tuple[tuple[int, int, int], ...]:
...
def digit_position(digit: int) -> LoShuPosition:
    ...
    if digit not in POSITION_BY_DIGIT:
        raise ValueError(f"Lo Shu digit must be from 1 to 9, got {digit!r}")
...
def scale_number() -> int:
    return LO_SHU_MAGIC_SUM * (LO_SHU_DECIMAL_CYCLE + 3) + 3
