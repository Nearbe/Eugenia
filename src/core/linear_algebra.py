...
class CoreMatrix(list):
    """List-backed matrix used by core/nucleus code."""
...
def to_matrix(values) -> CoreMatrix:
    """Convert scalar/vector/matrix-like values into CoreMatrix."""
...
def shape(values) -> tuple[int, ...]:
    ...
    if isinstance(first, Iterable) and not isinstance(first, (str, bytes)):
        return (len(rows), len(list(first)))
    return (len(rows),)
...
def dot(vector_a, vector_b) -> float:
    return sum(left * right for left, right in zip(to_vector(vector_a), to_vector(vector_b)))
...
def matmul(matrix_a, matrix_b) -> CoreMatrix:
    ...
    return CoreMatrix([[dot(row, col) for col in matrix_b_t] for row in to_matrix(matrix_a)])
